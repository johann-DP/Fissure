...
def print_system_status():
    ...
    # Last CSV timestamp
    stdin, stdout, stderr = client.exec_command(f"stat -c %y {remote_path}")
...
# ---------------------------------------------------------- 1. FETCH / LOAD
print_system_status()
fetch_remote_csv(hostname, port, username, password, local_path, remote_path, do_backup=True)
df = load_data(local_path, start_day_str, end_day_str)
...
# 1. Heures brutes d'extrêmes (désormais déjà converties en décimales)
h_min = min_times
h_max = max_times

# 2. Isolation du cluster principal (heures)
h_max_cluster = get_primary_peak_hours(h_max, window_half_width=window_half_width)
h_min_cluster = get_primary_peak_hours(h_min, window_half_width=window_half_width)
